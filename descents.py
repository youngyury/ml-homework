...
class BaseDescent:
    ...
    def calc_loss(self, x: np.ndarray, y: np.ndarray) -> float:
        ...
        :return: loss: float
        """
        return (1 / len(x)) * np.dot(y.T - self.predict(x), y - self.predict(x))
...
class Adam(VanillaGradientDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        self.v = self.beta_2 * self.v + (1 - self.beta_2) * np.dot(gradient, gradient)
        m = self.m / (1 - pow(self.beta_1, self.iteration))
        v = self.v / (1 - pow(self.beta_2, self.iteration))
        w = self.lr() * np.sqrt(m) / (np.sqrt(v) + self.eps)
        self.w = self.w - w
        self.iteration += 1
        return w
        # TODO: implement updating weights function
